  ...
  const mappedStoreProducts = useMemo(() => {
    ...
    return allProducts
      .filter(p => {
        ...
        return stockItems.some((si: any) => (String(si.store?.id) === String(store.id) || si.store?.slug === store.slug) && si.quantity > 0);
      })
      .map(p => {
        const stockEntry = p.store_stock.find((si: any) => (String(si.store?.id) === String(store.id) || si.store?.slug === store.slug) && si.quantity > 0);
        return {
          ...p,
          id: p.id,
